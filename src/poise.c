...
void ReadOLRModel(BODY *body, CONTROL *control, FILES *files, OPTIONS *options,
  ...
  if (lTmp >= 0) {
    ...
    } else if (!memcmp(sLower(cTmp), "kg24", 3)) {
      body[iFile - 1].iOLRModel = KG24;
    } else {
      if (control->Io.iVerbose >= VERBERR) {
        fprintf(stderr,
                "ERROR: Unknown argument to %s: %s."
                " Options are wk97, hm16, sms09, or kg24.\n",
                options->cName, cTmp);
      }
      LineExit(files->Infile[iFile].cIn, lTmp);
    }
...
double OLRKG24(BODY *body, int iBody, double T) {
  ...
  //
  // C0 from Byrne and Goldblatt 2013 doi:10.1002/2013GL058456
...
void fvAlbedoSeasonal(BODY *body, int iBody, int iDay) {
  ...
  for (iLat = 0; iLat < body[iBody].iNumLats; iLat++) {
    ...
    if (body[iBody].bCalcAB == 1 && body[iBody].iOLRModel != KG24) {
      if (body[iBody].iOLRModel == WK97) {
